...
# 1. Проверка и установка необходимых пакетов (Docker, Certbot, OpenSSL, Compose, Cron)
apt-get update -y
apt-get install -y curl wget ca-certificates openssl certbot cron
...
if command -v crontab &> /dev/null; then
    if ! crontab -l 2>/dev/null | grep -q "certbot renew"; then
        ...
        (crontab -l 2>/dev/null || true; echo "0 3 1 * * certbot renew --quiet && cd $(pwd) && $COMPOSE_CMD restart frontend") | crontab - 2>/dev/null || true
    fi
fi
